...
function googleAuthError(config, data) {
  ...
  const guidance =
    config.authType === 'service_account'
      ? 'Regenerate the service-account JSON key, update GOOGLE_SERVICE_ACCOUNT_JSON_BASE64, and restart the dev server.'
      : 'Regenerate the OAuth refresh token and update GOOGLE_OAUTH_REFRESH_TOKEN.'
