...
const card = defineModel<{ role: string; icon: string; desc: string }>({ required: true })
</script>
...
<template>
  <article class="sub-editor">
    <header class="sub-editor-header">
      ...
      <button type="button" class="icon-btn danger" aria-label="Remove card" @click="$emit('remove')">
